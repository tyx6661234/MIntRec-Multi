...
class MAG(nn.Module):
    ...
    def forward(self, text_embedding, visual, acoustic):
        ...
        hm_norm_ones = torch.ones(hm_norm.shape, device=hm_norm.device, requires_grad=True)
        hm_norm = torch.where(hm_norm == 0, hm_norm_ones, hm_norm)

        thresh_hold = (em_norm / (hm_norm + eps)) * self.beta_shift

        ones = torch.ones(thresh_hold.shape, device=thresh_hold.device, requires_grad=True)
...
class MAG_BertModel(BertPreTrainedModel):
    ...
    Return:
        ...
        # input head_mask has shape [num_heads] or [num_hidden_layers x num_heads x seq_length x seq_length]
...
class MAG_BERT(nn.Module):
    def __init__(self, args):
        ...
        # transformers 5.x loads on the meta device; modules whose weights are missing from the
        # checkpoint (the nn.LSTM/GRU inside AlignSubNet) lose their constructor-time
        # reset_parameters() and are left as uninitialized memory, so re-apply it explicitly.
