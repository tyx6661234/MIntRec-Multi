...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        # Allocate output tensor for causal_conv1d_fwd (arg5 'out' must be a Tensor, not None)
    ...
    def backward(ctx, dout):
        ...
        # Newer causal_conv1d requires pre-allocated output tensors and mutates in-place
        dconv1d_weight = torch.zeros_like(conv1d_weight, dtype=torch.float32)
        dconv1d_bias = torch.zeros_like(conv1d_bias, dtype=torch.float32) if conv1d_bias is not None else None
        causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out, None, None, None,
            dx, dconv1d_weight, dconv1d_bias, None, False
        )
