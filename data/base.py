...
class DataManager:
    ...
    def _get_multimodal_data(self, args):
        ...
        mm_data = {}
        for split, label_ids in [
            ('train', self.train_label_ids),
            ('dev', self.dev_label_ids),
            ('test', self.test_label_ids),
        ]:
            mm_data[split] = MMDataset(label_ids, text_data[split], video_data[split], audio_data[split])
            # Free each padded list as soon as its tensor is built to keep peak memory low.
            video_data[split] = None
            audio_data[split] = None
