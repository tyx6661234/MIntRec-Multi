...
class VideoDataset:
    ...
    def __padding(self, feat, video_max_length, padding_mode = 'zero', padding_loc = 'end'):
        ...
        if padding_mode == 'zero':
            pad = np.zeros([video_max_length - video_length, feat.shape[-1]], dtype=np.float32)
        elif padding_mode == 'normal':
            mean, std = feat.mean(), feat.std()
            pad = np.random.normal(mean, std, (video_max_length - video_length, feat.shape[1])).astype(np.float32)
