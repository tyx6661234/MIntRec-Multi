...
| 目标二 A/B 对照实验 | 🔄 **重跑中**（2026-08-29 23:06 首次启动的 dlf_a 在第 31 轮被中断；08-30 12:31 双链重启，见 §6） |
...
- 预计全程 ~2-2.5h（双链并行、每链两个方法串行）。

**判定**：dev F1 0.62→0.68+、acc 进入 69~71 即方向正确；A/B 取胜者。

### 6.2 后续（按胜者二阶微调，单变量，dev 判定）

- 若 A 胜：累积 10→8/16 微调；再试 `grad_clip` 0.6→0.8
- 若 B 胜：lr 3e-5→2e-5/5e-5 细扫
- R2 备忘：dlf 在纯 bs64（无累积、lr 1e-4）已达 69.21，若 A/B 均不达 70，可补一路"bs64 直跑 + lr 5e-5"作方案 C
...
**差距现状**：R1 bs16 最好 71.24 / 66.59（−1.4 / −2.8 至 L1）。实测 bs16 单 run 3-6 分钟，**网格预算充足**（v1 估 3h → 实际 1~2h/20 配置）。

**行动项**：
1. **阶段 1：配方对齐**（3 个 run，~20 分钟）
   - 管理器加梯度累积（复用 DLF/DDSE 的改动模式）：bs16 + `update_epochs`=8（等效 128）
   - lr 2e-5 → 3e-5（等效 batch 放大后同步上调）；patience 8→12
2. **阶段 2：dev 网格搜索**（15~20 个配置 × 单 seed，~1-2h，单变量推进）
   - `d_state` 8→16→32（Mamba 容量，预期收益最大）
   - `dst_feature_dim_nheads` [128,1]→[128,4]→[256,8]（当前注意力头数=1 明显偏小）
   - `conv1d_kernel` 3/3/3→5/5/5（EMOE/GsiT/DLF 均用 5）
   - 辅助损失消融：λ_rec/λ_ort/γ1/γ2 各减半或归零（DLF/DDSE 教训：验证辅助项是否稀释主任务）
   - `text_dropout` 0.3→0.4/0.5；lr 调度 ReduceLROnPlateau→warmup+线性
3. **阶段 3：5 seeds 定版**（~30 分钟）：最优配置 seeds 0-4，报 mean±std + best
...
[进行中] 目标二 A/B 双链重跑（dlf_a→ddse_a ‖ dlf_b→ddse_b）   ~2-2.5h  ──┐
                                                                        ├─→ 目标二定版（二阶微调 ~1h）
