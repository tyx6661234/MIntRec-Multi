...
| 目标三 TCS_Mamba 阶段 2 dev 网格 | 🔄 **运行中**（9 配置双链：结构变量链 d16/d32/h4/h8/k5 + 损失链 auxhalf/aux0/dp04/dp05） |
...
2. 🔄 **阶段 2：dev 网格搜索**（9 配置双链，单 seed 0，进行中）
   - 结构链：`d_state` 16 / 32、`dst_feature_dim_nheads` [128,4] / [256,8]、`kernel_size` 3→5
   - 损失/正则链：辅助损失 λ_rec/λ_ort/γ1/γ2 减半 / 归零、`text_dropout` 0.4 / 0.5
   - 判定：dev 提升方进入阶段 3；若全无效则回退 a2 直接 5 seeds
