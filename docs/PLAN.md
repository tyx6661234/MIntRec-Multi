...
| 目标三 TCS_Mamba 阶段 3 多种子定版 | ✅ **完成（结论反转）**：R1 基线 5-seed 均值 **71.01±0.64 / 67.39±0.49 反超 a2**(69.66±1.31/66.25±1.28)——累积配方对 TCS 无效且不稳，见 §7 |
...
3. ✅ **阶段 3：5 seeds 定版**（2026-08-31 完成，**结论反转**）：

   | 配置 | 5-seed test acc | 5-seed test F1 | best |
   |---|---|---|---|
   | **R1 基线（bs16 无累积 lr 2e-5）** | **71.01 ± 0.64** | **67.39 ± 0.49** | 71.46 / 67.80 |
   | a2（累积8 lr 3e-5） | 69.66 ± 1.31 | 66.25 ± 1.28 | 71.91 / 67.99 |

   **阶段 1~2 的"累积配方"在 5-seed 口径下是失败的**：a2 的 seed 0(71.91)是离群高位,其余 4 个 seed 全在 68.5~69.5,均值被基线反超 1.35 acc / 1.14 F1,且方差更大(±1.31 vs ±0.64)。**"拉大等效 batch + 提 lr"对 DLF/DDSE 有效、对 TCS(Mamba 系)无效甚至有害**——这是比配方本身更有价值的论文负结果。TCS 最优仍是原始 R1 配方：**71.01±0.64 / 67.39±0.49,较 L1(72.6/69.4)差 −1.6 / −2.0**。
4. **阶段 4（待用户决策）**：结构增强——CoSSM 上加 EMOE 式单模态蒸馏（引用其论文）；HLBF 低秩秩数调优。改动会改变论文方法定义，需用户确认后实施。
